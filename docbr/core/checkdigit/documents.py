...
class CPF(CheckDigit):
    """
    This class provides methods to validate Brazilian individual taxpayer identification numbers (CPF).
...
class CNPJ(CheckDigit):
    """
    This class provides methods to validate Brazilian company registration numbers (CNPJ).
    ...
    def __init__(self, docs: ndarray) -> None:
        ...
        self._attributes = {
            ...
            "matriz_filial": lambda x: where(
                array_slicer(x, 8, 11, dtype=int) == 1, "matriz", "filial"
            ),
        }


class CNH(CheckDigit):
    """
    This class provides methods to validate Brazilian driver's license numbers (CNH).
        ...
        for sequence, position in self._sequence:
            ...
            if len(cache) > 0:
                digit = where(
                    cache[0] < 10,
                    digit,
                    where(digit - 2 < 0, digit + 9, digit - 2),
                )
...
class TituloEleitor(CheckDigit):
    """
    This class provides methods to validate Brazilian voter registration numbers (Título de Eleitor).
    ...
    def _federal_unit_validator(
        self, digs: ndarray, dig_fu: tuple[int, int]
    ) -> ndarray:
        """
        Validates the federal unit digits in a Brazilian voter registration number.

        :param digs: The array of digits in the voter registration number.
        :type digs: numpy.ndarray

        :param dig_fu: A tuple containing the index positions of the federal unit digits.
        :type dig_fu: tuple[int, int]
    ...
    def _validation_process(self, digits: ndarray) -> ndarray:
        """
        Checks the validity of the input data using the validation process of the document type.
...
class PIS(CheckDigit):
    """
    This class provides methods to validate Brazilian social security numbers for workers (PIS).
...
class Certidao(CheckDigit):
    """
    This class provides methods to validate Brazilian birth, marriage or death certificates (Certidão).
...
class Renavam(CheckDigit):
    """
    This class provides methods to validate Brazilian vehicle registration numbers (Renavam).
